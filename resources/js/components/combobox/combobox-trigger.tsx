...
export const ComboBoxTrigger = forwardRef<HTMLButtonElement, ComboBoxTriggerProps>(
    ({ placeholder = 'Select…', disabled, className, ...props }, ref) => {
        const { selectedOptions, multiple, clearAll, removeOption } = useComboBoxContext();
        ...
        return (
            <button
                ...
                className={cn(
                    ...
                    'transition-colors focus:ring-1 focus:ring-ring focus:outline-none',
                    'disabled:cursor-not-allowed disabled:opacity-50',
                    // Multi-select has different inner layout
                    multiple && hasValue ? 'flex-wrap gap-1.5 py-1.5' : 'flex-nowrap',
                    className,
                )}
                {...props}
            >
                ...
                <span className="flex min-w-0 flex-1 flex-wrap items-center gap-1.5">
                    {hasValue ? (
                        multiple ? (
                            <ComboBoxMultiValue options={selectedOptions} onRemove={removeOption} disabled={disabled} />
                        ) : (
